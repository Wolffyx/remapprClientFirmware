...
const ZMK_CAPABILITIES: Capabilities = {
    ...
    // Studio unlock is opt-in on ZMK: without a &studio_unlock binding in the
    // keymap there is no way to unlock at all, so the unlock screen has to say
    // so. The text and the link live here, with the firmware they describe.
    unlockHint: {
        message:
            "If studio unlocking hasn't been added to your keymap or a combo, this keyboard cannot be unlocked. See:",
        docsUrl: 'https://zmk.dev/docs/keymaps/behaviors/studio-unlock',
        docsLabel: 'Studio Unlock Behavior',
    },
...
export class ZmkKeyboardService implements KeyboardService {
    ...
    // Combo sideload is built but NOT exposed: ./sideload.ts can read a .keymap
    // file and surface its combos as display-only picker entries, but they are
    // informational only (combos are compile-time devicetree and can't be
    // assigned), so the button earns no place in the toolbar today. Declaring
    // the facade is all it takes to turn it on:
    //
    //   public readonly sideload: SideloadApi = createZmkSideload()
    //
    // plus the two imports (`SideloadApi` from '@firmware/sideload',
    // `createZmkSideload` from './sideload'). Nothing else needs changing — the
    // app renders one button per declared format.
