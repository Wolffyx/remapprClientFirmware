...
import type { ConfigEditingApi } from '@firmware/configEditing'
import type { LinkLimitKnob } from '@firmware/configEditing'
import type { Limits } from '@firmware/clients/remappr/protocol'
...
const MOCK_CAPABILITIES: Capabilities = {
    ...
    saveMode: 'none',
    // Simulated device: never discovered on a bus, so the app must skip anything
...
export class MockKeyboardService implements KeyboardService, ConfigEditingApi {
    ...
    public readonly configBridge: ConfigBridgeApi = {
        raiseKeymap: (layers, prevConfig) =>
            raiseMockToConfig(layers, prevConfig),
    }
